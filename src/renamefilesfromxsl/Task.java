            ...
            for (int x = 0; x < ficheros.length; x++) {
                ...
                try {
                    ...
                    if (Utilerias.buscaCadena(ficheros[x].getName(), ".jpg")) {
                        int pos = Utilerias.existeCadenaArray2(Utilerias.quitaExtension(ficheros[x].getName()), nombres_xls);
                        //System.out.println("pos---- " + pos + "index--");
                        if (pos != -1) {
                            String nombre_destino = nombres_xls[pos][1];
                            rutafinal = ruta_destino + "/" + nombre_destino;
                             if (Utilerias.copiaArchivo(ruta_origen + "/" + file_name,
                             rutafinal + ".jpg")) { index++; }
                             ///System.out.println(nombre_destino);
                            if (nombres_xls[pos][0].equals(nombres_xls[pos + 1][0])) {
                                
                                  if (Utilerias.copiaArchivo(ruta_origen + "/" + file_name, ruta_destino + "/" + nombres_xls[pos + 1][1] + ".jpg")) { 
                                      index++;
                                  }
                            }
                     ...
                     this.ta_logs.append( "------ERROR: No encontrado  " +e+"\n");
                }
